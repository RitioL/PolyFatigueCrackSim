...
CC
     ...
     3 JMAC(*),JMATYP(*),DROT(3,3),ORI(3,3),LXFEM(3)
      ...
      COMMON /monitor/ num, ipad, prev

C     Find out the biggest value
      SIG1 = STATEV(9*12+1)
      KMAX = 1
      DO I = 2, 12
         IF(STATEV(9*12+I).GT.SIG1) THEN
            SIG1 = STATEV(9*12+I)
            KMAX = I
         END IF
      END DO
...
c   lay the problem. The UMAT does not restrict the current slip to be a 
...
C
         ...
         IDNOR=3*NSLPTL
CFIX--  Store the normals from the previous step
CFIXA2024
C         IDNOR2=11*NSLPTL
CFIXB2024
         IDDIR=6*NSLPTL
